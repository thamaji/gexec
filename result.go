...
func (r *Result) Scan(v interface{}) error {
	...
	if rv.Kind() != reflect.Ptr {
		return errors.New("gexec: Scan(non-pointer " + rv.Type().String() + ")")
	}
...
func decode(s string, v reflect.Value) error {
	...
	case reflect.Bool:
		...
		return errors.New("gexec: cannot decode " + v.Kind().String() + ": " + s)
	...
	default:
		...
		return errors.New("gexec: unsupported type: " + v.Kind().String())
	}
